...
var ReactDatatable = function (_Component) {
  ...
  _createClass(ReactDatatable, [
    ...
    value: function addFilterDropdown(e) {
      ...
      if (value !== "All") {
        Object.defineProperty(filter, key, {value, enumerable:true})
      } else {
        delete this.state.fetchFilters[Object.keys(this.state.filter)[0]];
      }
        this.setState({
    ...
    value: function addFilterSwitch(e) {
      ...
      let value;
      let filter = new Object();
      const trueElmt = document.getElementById(name+"-true");
      const falseElmt = document.getElementById(name+"-false");

      if (e.target.id === trueElmt.id) {
        trueElmt.classList.contains("is-success") ? 
        trueElmt.classList.remove("is-success") : 
        trueElmt.classList.add("is-success");
        falseElmt.classList.remove("is-danger");
      } else if (e.target.id === falseElmt.id) {
        falseElmt.classList.contains("is-danger") ? 
        falseElmt.classList.remove("is-danger") : 
        falseElmt.classList.add("is-danger");
        trueElmt.classList.remove("is-success");
      }
