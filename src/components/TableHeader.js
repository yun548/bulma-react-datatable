...
export default function TableHeader(props){
    if(props.config.show_length_menu == true
      ...
      return (
        <div className="level notification mb-0" id={(props.id) ? props.id + "-table-head" : ""}>
              ...
              {(props.config.show_length_menu) ? (
                <div className="input-group asrt-page-length">
                  {(includes(props.config.language.length_menu, '_MENU_')) ? (
                    <select type="text" className="input" style={style.table_size_dropdown}
          ...
          <div className="level-right">
            <div className="level-item">
              {(props.config.show_filter) ? (
                <div className="table_filter control" style={style.table_filter}>
                  <input
                    type="search"
                    className="input"
              ...
              <div className="table_tools" style={style.table_tool}>
                {(props.config.button.excel) ? (
                  <button className="button is-primary buttons-excel"
                    tabIndex="0"
                    ...
                    <span>
                      <i className="iconify" data-icon="fa:file-excel-o"></i>
                    </span>
                  </button>
                ) : null}
                {(props.config.button.csv) ? (
                  <button className="button is-primary buttons-csv"
                    tabIndex="0"
                    aria-controls="configuration_tbl"
                    title="Export to CSV"
                    style={style.table_tool_btn}
                    ...
                    <span>
                      <i className="iconify" data-icon="fa:file-text-o"></i>
                    </span>
                  </button>
                ) : null}
                {(props.config.button.print) ? (
                  <button className="button is-primary buttons-pdf"
                    tabIndex="0"
                    aria-controls="configuration_tbl"
                    title="Export to PDF"
                    style={style.table_tool_btn}
                    ...
                    <span>
                      <i className="iconify" data-icon="fa:print"></i>
                    </span>
                  </button>
                ) : null}
                {(props.config.button.extra==true) ? (
                  props.extraButtons.map((elem,index)=>{
                      ...
                      return (
                          <button className={elem.className ? elem.className : "button is-primary buttons-pdf"}
                            tabIndex="0"
                            aria-controls="configuration_tbl"
